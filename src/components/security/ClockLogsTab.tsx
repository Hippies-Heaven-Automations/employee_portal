...
import { Search, Plus, Pencil, Trash2, Loader2, Clock } from "lucide-react";
import ClockLogForm from "./ClockLogForm";
...
export default function ClockLogsTab() {
  ...
  const [selectedLog, setSelectedLog] = useState<ClockLog | null>(null);
  ...
  const fetchStoreEmployees = async () => {
    const { data, error } = await supabase
      ...
      .ilike("employee_type", "store") // 👈 case-insensitive match
  ...
  const fetchLogs = async () => {
    ...
    if (error) notifyError(`Error loading clock logs: ${error.message}`);
    else setLogs((data as ClockLog[]) || []);
    setLoading(false);
  };
  ...
  const onEdit = (log: ClockLog) => {
    setSelectedLog(log);
    setIsFormOpen(true);
  };

  const onDelete = (id: string) => {
    confirmAction("Delete this clock log? This cannot be undone.", async () => {
      const { error } = await supabase.from("clock_logs").delete().eq("id", id);
      if (error) notifyError(`Error deleting log: ${error.message}`);
      else {
        notifySuccess("🗑️ Clock log deleted.");
        setLogs((prev) => prev.filter((l) => l.id !== id)); // instant removal
      }
    });
  };
    ...
    <section className="animate-fadeInUp text-gray-700">
      ...
      <div className="flex flex-col sm:flex-row items-start sm:items-center justify-between mb-4 gap-3">
        <div className="flex items-center gap-2">
          <Clock size={22} className="text-hemp-forest" />
          <h2 className="text-xl font-bold text-hemp-forest">Clock In / Out Logs</h2>
        </div>
        <Button
          onClick={onAdd}
