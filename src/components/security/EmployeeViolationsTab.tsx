...
import { Search, Plus, Pencil, Trash2, Loader2, AlertOctagon } from "lucide-react";
...
export default function EmployeeViolationsTab() {
  ...
  const fetchLogs = async () => {
    ...
    if (error) notifyError(`Error loading violations: ${error.message}`);
    else setLogs((data as Violation[]) || []);
    setLoading(false);
  };
  ...
  const filtered = useMemo(() => {
    ...
    return logs.filter(
      (l) =>
        ...
        (l.employee_id && (empMap.get(l.employee_id)?.toLowerCase().includes(q) || false))
    );
  ...
  const onEdit = (log: Violation) => {
    setSelectedLog(log);
    setIsFormOpen(true);
  };

  const onDelete = (id: string) => {
    confirmAction("Delete this employee violation record?", async () => {
      const { error } = await supabase.from("employee_violations").delete().eq("id", id);
      if (error) notifyError(`Error deleting record: ${error.message}`);
      else {
        notifySuccess("🗑️ Violation record deleted.");
    ...
    <section className="animate-fadeInUp text-gray-700">
      ...
      <div className="flex flex-col sm:flex-row items-start sm:items-center justify-between mb-4 gap-3">
        <div className="flex items-center gap-2">
          <AlertOctagon size={22} className="text-hemp-forest" />
          <h2 className="text-xl font-bold text-hemp-forest">Employee Violations</h2>
        </div>
        <Button
          onClick={onAdd}
      ...
      <div className="flex flex-col md:flex-row md:items-center md:justify-between gap-3 mb-5 bg-white/70 backdrop-blur-md border border-hemp-sage/40 rounded-xl p-4 shadow-sm">
        <div className="relative w-full md:w-96">
          ...
          <input
            type="text"
            placeholder="Search by employee, description, or notes..."
            value={searchTerm}
            onChange={(e) => {
      ...
      <div className="bg-white border border-hemp-sage rounded-lg shadow-sm overflow-hidden">
        {loading ? (
          <div className="p-6 text-center text-gray-500 inline-flex items-center gap-2 justify-center">
            <Loader2 className="animate-spin" /> Loading violations...
          </div>
        ) : pageItems.length === 0 ? (
          <div className="p-6 text-center text-gray-500">No violations found.</div>
        ) : (
                  ...
                  return (
                    ...
                    >
                      ...
                      <td
                        className="px-4 py-3"
                        dangerouslySetInnerHTML={{
                          __html: highlightMatch(row.notes || ""),
                        }}
                      />
