...
import { Search, Plus, Pencil, Trash2, Loader2, Wallet } from "lucide-react";
import CashRemovalForm from "./CashRemovalForm";

interface CashRemovalLog {
  id: string;
  date: string;
  time: string;
...
export default function CashRemovalTab() {
  const [logs, setLogs] = useState<CashRemovalLog[]>([]);
  ...
  const [selectedLog, setSelectedLog] = useState<CashRemovalLog | null>(null);
  ...
  const fetchLogs = async () => {
    setLoading(true);
    const { data, error } = await supabase
      .from("cash_removal_logs")
      .select("*")
      .order("date", { ascending: false })
      .order("time", { ascending: false });

    if (error) notifyError(`Error loading cash removal logs: ${error.message}`);
    else setLogs((data as CashRemovalLog[]) || []);
    setLoading(false);
  };
  ...
  const onEdit = (log: CashRemovalLog) => {
    setSelectedLog(log);
    setIsFormOpen(true);
  };

  const onDelete = (id: string) => {
    confirmAction("Delete this cash removal log? This cannot be undone.", async () => {
      const { error } = await supabase.from("cash_removal_logs").delete().eq("id", id);
      if (error) notifyError(`Error deleting log: ${error.message}`);
      else {
        notifySuccess("🗑️ Cash removal log deleted.");
    ...
    <section className="animate-fadeInUp text-gray-700">
      ...
      <div className="flex flex-col sm:flex-row items-start sm:items-center justify-between mb-4 gap-3">
        <div className="flex items-center gap-2">
          <Wallet size={22} className="text-hemp-forest" />
          <h2 className="text-xl font-bold text-hemp-forest">Cash Removal Logs</h2>
        </div>
        <Button
          onClick={onAdd}
