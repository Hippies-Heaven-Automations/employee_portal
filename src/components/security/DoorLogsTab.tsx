...
import { Search, Plus, Pencil, Trash2, Loader2, DoorOpen } from "lucide-react";
import DoorLogForm from "./DoorLogForm";
...
export default function DoorLogsTab() {
  ...
  const [selectedLog, setSelectedLog] = useState<DoorLog | null>(null);

  // store employees for display & dropdowns
  ...
  const fetchStoreEmployees = async () => {
    const { data, error } = await supabase
        ...
        .eq("employee_type", "Store")  // only store employees
  ...
  const fetchLogs = async () => {
    ...
    if (error) notifyError(`Error loading door logs: ${error.message}`);
    else setLogs((data as DoorLog[]) || []);
    setLoading(false);
  };
  ...
  // Filter + search (name/location/note) — name via empMap
  const filtered = useMemo(() => {
    ...
    return logs.filter((l) => {
      ...
      return (
        ...
        (l.door_location || "").toLowerCase().includes(q) ||
  ...
  const onEdit = (log: DoorLog) => {
    setSelectedLog(log);
    setIsFormOpen(true);
  };

  const onDelete = (id: string) => {
    confirmAction("Delete this door log? This cannot be undone.", async () => {
      const { error } = await supabase.from("door_logs").delete().eq("id", id);
      if (error) notifyError(`Error deleting log: ${error.message}`);
      else {
        notifySuccess("🗑️ Door log deleted.");
        fetchLogs();
      }
    });
  };
    ...
    <section className="animate-fadeInUp text-gray-700">
      ...
      <div className="flex flex-col sm:flex-row items-start sm:items-center justify-between mb-4 gap-3">
        <div className="flex items-center gap-2">
          <DoorOpen size={22} className="text-hemp-forest" />
          <h2 className="text-xl font-bold text-hemp-forest">Door Logs</h2>
        </div>
        <Button
          onClick={onAdd}
      ...
      <div className="flex flex-col md:flex-row md:items-center md:justify-between gap-3 mb-5 bg-white/70 backdrop-blur-md border border-hemp-sage/40 rounded-xl p-4 shadow-sm">
        <div className="relative w-full md:w-96">
          ...
          <input
            type="text"
            placeholder="Search by employee, location, or note..."
            value={searchTerm}
            onChange={(e) => {
      ...
      <div className="bg-white border border-hemp-sage rounded-lg shadow-sm overflow-hidden">
        {loading ? (
          <div className="p-6 text-center text-gray-500 inline-flex items-center gap-2 justify-center">
            <Loader2 className="animate-spin" /> Loading door logs...
          </div>
        ) : pageItems.length === 0 ? (
                      ...
                      >
                        ...
                        <td
                          ...
                          dangerouslySetInnerHTML={{
                            __html: highlightMatch(row.door_location || ""),
                          }}
                        />
                        <td
                          className="px-4 py-3"
                          dangerouslySetInnerHTML={{
                            __html: highlightMatch(row.note || ""),
                          }}
                        />
                ...
                return (
                  <div key={row.id} className="p-4 bg-white">
                    <div className="flex justify-between items-start mb-2">
                      <div>
                        <h3 className="text-base font-semibold text-hemp-forest">
                      ...
                      <span className="px-2 py-1 text-xs rounded bg-hemp-sage/30 text-hemp-forest">
                        {row.door_location}
                      </span>
                    </div>
                    <p
                      className="text-sm text-gray-700 mb-2"
                      dangerouslySetInnerHTML={{ __html: highlightMatch(row.note || "") }}
                    />
                    <p className="text-sm">
                      {row.footage_link ? (
                        <a
      ...
      {isFormOpen && (
        <DoorLogForm
            storeEmployees={storeEmployees}   // ✅ pass here
            initial={selectedLog}
