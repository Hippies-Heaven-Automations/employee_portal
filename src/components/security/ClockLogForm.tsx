...
type ClockNote = "Clock In" | "Clock Out";

interface ClockLogInput {
  id?: string;
...
export default function ClockLogForm({
  initial,
  storeEmployees,
  onClose,
  onSaved,
}: {
  initial: Partial<ClockLogInput> | null; // ✅ replaced any | null
...
const [form, setForm] = useState<ClockLogInput>({
  ...
  note: (initial?.note as ClockNote) ?? "Clock In",
});
  ...
  if (initial) {
    setForm({
      id: initial.id ?? undefined,
      date: initial.date ?? toISODate(now),              // ✅ fallback to today's date
      time: initial.time?.slice(0, 5) ?? toHHMM(now),    // ✅ safe slice, fallback to now
      employee_id: initial.employee_id ?? null,          // ✅ null if missing
      footage_link: initial.footage_link ?? "",
      note: (initial.note as ClockNote) ?? "Clock In",   // ✅ safe default
    });
  }
}, [initial, now]);


 const onChange = <K extends keyof ClockLogInput>(
  key: K,
  value: ClockLogInput[K]
  ...
  const handleSave = async () => {
    ...
    if (form.id) {
      const { error } = await supabase.from("clock_logs").update(payload).eq("id", form.id);
      errorMsg = error?.message || null;
    } else {
      const { error } = await supabase.from("clock_logs").insert(payload);
      errorMsg = error?.message || null;
    }
      ...
      <div className="w-full max-w-xl bg-white rounded-2xl shadow-xl border border-hemp-sage/50 overflow-hidden">
        ...
        <div className="flex items-center justify-between px-5 py-4 border-b border-hemp-sage/40 bg-hemp-mist/40">
          <h3 className="text-lg font-semibold text-hemp-forest">
            {form.id ? "Edit Clock Log" : "Add Clock Log"}
          </h3>
          <button
            onClick={onClose}
        ...
        <div className="p-5 grid grid-cols-1 sm:grid-cols-2 gap-4">
          ...
          <div className="flex flex-col">
            ...
            <select
              value={form.note}
              onChange={(e) => onChange("note", e.target.value as ClockNote)}
              className="rounded-lg border border-hemp-sage/50 bg-white/60 px-3 py-2 focus:ring-2 focus:ring-hemp-green focus:border-hemp-green"
            >
